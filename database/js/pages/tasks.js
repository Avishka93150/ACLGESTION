...
async function openTaskDetail(taskId) {
    try {
        ...
        openModal(`<span style="color: ${priorityColors[task.priority]}; font-weight: 600;">${esc(task.title)}</span>`, `
            <div class="task-detail">
                <div class="task-detail-main">
                    <div class="task-detail-section">
                        <label><i class="fas fa-align-left"></i> Description</label>
                        <div class="task-description-box" onclick="editTaskDescription(${task.id}, this)">
...
async function toggleTaskComplete(taskId, completed) { try { await API.updateTask(currentBoard, taskId, { is_completed: completed }); toast(completed ? 'Terminée' : 'Rouverte', 'success'); closeModal(); openBoard(currentBoard); } catch (error) { toast(error.message, 'error'); } }
...
function editTaskDescription(taskId, element) {
    const currentDesc = element.textContent.trim(); const isPlaceholder = element.querySelector('.text-muted');
    element.innerHTML = `<textarea id="edit-desc-textarea" rows="4">${isPlaceholder ? '' : esc(currentDesc)}</textarea><div class="edit-actions"><button class="btn btn-sm btn-primary" onclick="saveTaskDescription(${taskId})">Enregistrer</button><button class="btn btn-sm btn-outline" onclick="openTaskDetail(${taskId})">Annuler</button></div>`;
