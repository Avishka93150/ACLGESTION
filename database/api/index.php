...
$subaction = $segments[3] ?? null;  // Alias pour compatibilité messaging
    ...
    <div style='max-width: 600px; margin: 0 auto; padding: 20px;'>
        ...
        case 'tasks':
            ...
            if ($method === 'GET' && $id && is_numeric($id) && !$action) {
                ...
                // Si pas de membres assignés au tableau, fallback sur tous les membres de l'hôtel
                if (empty($members)) {
                    $members = db()->query(
                        "SELECT u.id, u.first_name, u.last_name, u.role FROM users u
                         JOIN user_hotels uh ON u.id = uh.user_id
                         WHERE uh.hotel_id = ? AND u.status = 'active'
            ...
            if ($method === 'PUT' && $id && $action === 'checklist' && $subId) {
                ...
                db()->execute(
                    "UPDATE task_checklists SET is_checked = ?, checked_by = ?, checked_at = ? WHERE id = ?",
                    [$isChecked ? 1 : 0, $isChecked ? $user['id'] : null, $isChecked ? date('Y-m-d H:i:s') : null, $subId]
                );
        ...
        case 'messaging':
            ...
            if ($id === 'conversations' && $action && is_numeric($action) && $subaction === 'read' && $method === 'PUT') {
                $user = require_auth();
                $convId = (int)$action;
        ...
        case 'closures':
            ...
            if ($method === 'GET' && $id === 'daily' && $action && is_numeric($action) && $subaction) {
                $user = require_auth();
                $hotelId = intval($action);
                $dateStr = $subaction;
            ...
            if ($method === 'PUT' && $id === 'daily' && $action && is_numeric($action) && $subaction === 'validate') {
                $user = require_auth();
                if (!in_array($user['role'], ['admin', 'groupe_manager'])) json_error('Accès refusé', 403);
        ...
        case 'revenue':
            ...
            if ($method === 'POST' && $id === 'fetch-rates') {
                ...
                $dateTo = $data['date_to'] ?? date('Y-m-d', strtotime('+30 days'));
                ...
                $xoteloKey = $hotel['xotelo_hotel_key'] ?? null;
                if (empty($xoteloKey)) {
                ...
                // Générer les dates à requêter (max 14 jours pour limiter les appels API)
                ...
                // Supprimer les anciens tarifs pour cette période
                try {
                    db()->execute(
                ...
                // Appeler l'API Xotelo pour chaque hôtel ET chaque date
                    ...
                    foreach ($dates as $checkDate) {
                        ...
                        if ($httpCode === 200 && $response) {
                            ...
                            if (!empty($xoteloData['result']['rates'])) {
                                $currency = $xoteloData['result']['currency'] ?? 'EUR';
                                
                                foreach ($xoteloData['result']['rates'] as $rate) {
                                    // Chaque rate contient: code, name, rate, tax
                                    $otaName = $rate['name'] ?? $rate['code'] ?? 'unknown';
                                    $rateAmount = $rate['rate'] ?? 0;
                                    $tax = $rate['tax'] ?? 0;
            ...
            if ($method === 'GET' && $id === 'rates' && $action && is_numeric($action)) {
                ...
                $where = "hotel_id = ? AND check_date BETWEEN ? AND ?";
                ...
                json_out(['success' => true, 'rates' => $rates]);
            }
