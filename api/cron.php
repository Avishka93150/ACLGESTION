...
function checkMaintenanceAlerts() {
    ...
    foreach ($tickets2days as $t) {
        ...
        $message .= "📍 Localisation: " . (!empty($t['room_number']) ? $t['room_number'] : 'Parties communes') . "\n";
        $message .= "🏷️ Catégorie: {$t['category']}\n";
        $message .= "⚡ Priorité: {$t['priority']}\n";
        $message .= "📝 Description: {$t['description']}\n";
        $message .= "📅 Créé le: {$t['created_at']}\n";
        $message .= "👤 Signalé par: " . (!empty($t['reporter_name']) ? $t['reporter_name'] : 'Inconnu') . "\n\n";
    ...
    foreach ($tickets5days as $t) {
        ...
        $message .= "⚡ Priorité: {$t['priority']}\n";
        $message .= "📝 Description: {$t['description']}\n";
        $message .= "👷 Pris en charge par: " . (!empty($t['assigned_to_name']) ? $t['assigned_to_name'] : 'Non assigné') . "\n";
    ...
    foreach ($ticketsOverdue as $t) {
        ...
        $message .= "Assigné à: " . (!empty($t['assigned_to_name']) ? $t['assigned_to_name'] : 'Non assigné') . "\n";
        $message .= "Description: {$t['description']}\n";
        $message .= "Pris en charge le: {$t['assigned_at']}\n\n";
...
function cleanupOldData() {
    ...
    // Supprimer l'historique des tarifs Xotelo ancien (> 90 jours)
    try {
        $deleted = db()->execute("DELETE FROM xotelo_rates_history WHERE fetched_at < DATE_SUB(NOW(), INTERVAL 90 DAY)");
        echo "  Historique tarifs Xotelo supprimé (> 90 jours)\n";
    } catch (Exception $e) {
        // Table n'existe peut-être pas
    }
    
    // Supprimer les logs API Xotelo anciens (> 30 jours)
    try {
        db()->execute("DELETE FROM xotelo_api_logs WHERE created_at < DATE_SUB(NOW(), INTERVAL 30 DAY)");
...
function sendMaintenanceAlertEmail($to, $subject, $ticket, $alertType = '2days') {
    ...
    $priorityLabel = isset($priorityLabels[$ticket['priority']]) ? $priorityLabels[$ticket['priority']] : $ticket['priority'];
    $roomInfo = !empty($ticket['room_number']) ? $ticket['room_number'] : 'Parties communes';
...
// =============================================================================
// REVENUE MANAGEMENT - Actualisation automatique des tarifs Xotelo
// =============================================================================

/**
 * Actualise les tarifs Xotelo pour tous les hôtels configurés.
 * À exécuter tous les jours à 06h00.
 * 
 * Configuration dans Plesk :
 * php /chemin/api/cron.php revenue
